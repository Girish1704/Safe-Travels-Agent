...
1. 完了したら、左メニューから **[テーブル (1)]** を選択し、**[Excel または .CSV ファイルで作成する (2)]** をクリックします。

   ![](../media/leav-man-e1-g-2.png)

   > **注:** 権限がないというメッセージが表示された場合は、**[切り替えて作成する]** をクリックしてください。
...
1. **[Excel または .CSV ファイルをインポート]** ウィンドウで、テーブルが含まれていることを確認し、**[インポート]** をクリックします。
...
1. プロビジョニングが完了したら、**[テーブル (1)]** を開いて **[従業員 (2)]** テーブルが一覧に表示されていることを確認します。論理**プレフィックス (3)** も確認してください。このプレフィックスはテーブルを一意に識別するもので、今後の自動化に使用されます (このラボではこれ以上は使用しません)。

   ![](../media/ex1-travel-g4.png)

   > **ヒント:** **従業員** テーブルが表示されない場合は、**ODL_User <inject key="Deployment ID" enableCopy="false"></inject> の環境**に切り替えていることを確認してください。
...
1. **[Microsoft Copilot Studio へようこそ]** 画面で、既定の**国/地域**の選択のまま **[開始する]** をクリックして続行します。
...
1. Copilot Studio で **[エージェント (1)]** をクリックし、**[安全な旅行 (2)]** テンプレート カードを選択します。
...
1. 次のウィンドウで、**[名前 (1)]** フィールドと **[説明 (2)]** フィールドに以下の詳細を入力し、**[作成 (3)]** をクリックします。
   ...
   | 項目 | 値 |
   |-----|-------|
   | 名前 | `Safe Travels Agent` |
   | 説明 | `従業員の旅行計画、旅行規定、旅行に関するガイダンスなどをサポートする旅行アシスタント エージェント` |

   ![](../media/ex1-travel-g8-jp.png)

1. **[作成]** をクリックした後、緑色の成功バナー **「エージェントがプロビジョニングされました。」** が表示され、**[概要]** タブが読み込まれることを確認します。

   ![](../media/saf-tra-cor-v2-g6.png)

   > **テンプレートのメリット:** 安全な旅行 には出張フローとナレッジが事前設定されており、セットアップ時間を短縮できます。
...
1. **[ナレッジの追加]** 画面で、**[選択して参照]** をクリックしてナレッジ ファイルをアップロードします。
...
1. ファイルが正常にアップロードされたら、**[エージェントに追加する]** をクリックして、このドキュメントをエージェントのナレッジ ソースとして含めます。
...
1. **Copilot Studio** に移動し、**[エージェント (1)]** をクリックして、**[+ 空のエージェントを作成する (2)]** を選択して専門化された Leave Manager エージェントを作成します。
...
1. 次のウィンドウで、**[名前 (1)]** フィールドと **[説明 (2)]** フィールドに以下の詳細を入力し、**[保存 (3)]** を選択します。
   ...
   | 項目 | 値 |
   |-----|-------|
   | 名前 | `Leave Manager Agent` |
   | 説明 | `このエージェントは、全従業員の休暇、休暇残日数、休暇履歴を追跡し、新たな休暇申請を承認または却下する役割を担います。` |

   ![](../media/saf-tra-cor-v2-g10-jp.png)
...
1. 以下の指示を設定し、追加後に **[保存]** をクリックします。
   ...
   | 項目 | 値 |
   |-----|-------|
   | 指示 | `従業員の休暇を管理します。休暇残日数を追跡し、残日数に基づいて休暇の申請または却下を行います。` |

   ![](../media/saf-tra-cor-v2-g12-jp.png)

1. **[概要 (1)]** タブで **[ナレッジの追加 (2)]** をクリックして、エージェントの休暇管理機能を強化する組織データ ソースを含めます。

   ![](../media/ex2-travel-g70.png)

1. **[`選択して参照]** をクリックして、エージェントのナレッジの基盤となる休暇管理ドキュメントをアップロードします。
...
1. 必要な休暇ポリシーおよびトラッキング ファイルをアップロードしたら、**[エージェントに追加する]** をクリックして、権威あるナレッジ ソースとして統合します。
...
1. テスト チャットで次の**プロンプト (1)** を入力し、**[送信 (2)]** を選択します。
   ...
   ![](../media/ex1-travel-g16-jp.png)

1. エージェントによって生成された**応答**が期待通りであることを確認します。
   
   ![](../media/ex1-travel-g17-jp.png)
...
1. 「Teams を知ってもらう」の画面が表示された場合は、**[はじめる]** をクリックして続行します。

   ![](../media/ex1-travel-g22.png)

1. 公開が完了したら、**[チャネル]** セクションに移動してエージェントの展開チャネルを設定します。

   ![](../media/ex1-travel-g28-jp.png)

1. チャネル インターフェースに利用可能な展開オプションが表示されます。**[Teams および Microsoft 365 Copilot]** を選択してエージェントを Teams に展開します。

   ![](../media/ex1-travel-g29-jp.png)

   > **チャネルのメリット:** Microsoft Teams に展開することで、ユーザーは使い慣れたコラボレーション環境からアクセスできるようになり、導入と利用が促進されます。

1. **[Teams および Microsoft 365 Copilot]** ウィンドウで、**[エージェント を Microsoft 365 Copilot で使用可能にする (1)]** チェックボックスを選択し、**[チャネルを追加する (2)]** をクリックします。

   ![](../media/ex1-travel-g30.png)

1. チャネルが正常に追加されると、公開を確認するダイアログが表示されます。**[公開する]** をクリックしてください。

   ![](../media/sfimg8.png)

1. 公開が完了したら、**[Teams でエージェントを表示する]** オプションを選択して、Teams にエージェントを追加します。

   ![](../media/cor2-gs-g2.png)

1. Teams デスクトップ アプリのダウンロードを促すメッセージが表示された場合は、**[代わりに Web アプリを使用]** をクリックしてブラウザーで Teams を引き続き使用します。
